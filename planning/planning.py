...
def plan_bike_recharging():
    ...
    stat = None
    for s in stations:
        c = sum(stations[s][slot]=="empty" for slot in stations[s])
        print("COUNT", c)
    ...
    for g in stations[stat]:
        if stations[stat][g] == "empty":
            stations[stat][g] = "RESERVED"
            return stat, g
    return None, None
...
def retrieve_station_status():
    ...
    for table in tables:
        for record in table.records:
            ...
            if station_id not in stations: stations[station_id] = {}

            for i in range(1, N_SLOT+1):
                stations[station_id][f"s{i}"]=record.values.get(f"s{i}")
    for s in stations:
        ...
        for record in table.records:
            ...
            if record_time > last_time:
                ...
                if record_time > current_max_time:
                    ...
                    # Solo qui aggiorniamo la globale per il prossimo ciclo
                last_time = current_max_time


"""
def planner_decide(bike_id, alert_type):
    print(f"[PLANNING] Analizzando alert {alert_type} per bici {bike_id}")

    plan = None

    if alert_type == "OUT_OF_RANGE":
        # Piano: Bisogna bloccare il motore per sicurezza e avvisare il recupero
        plan = {
            "action": "LOCK_MOTOR",
            "message": f"Bici: {bike_id} fuori zona consentita"
        }

    elif alert_type == "LOW_BATTERY":
        # Piano: Mettere la bici in stato 'Maintenance' e non renderla prenotabile
        plan = {
            "action": "SET_MAINTENANCE_MODE",
            "message": f"Batteria scarica bici: {bike_id}"
        }

    elif alert_type == "AVAILABLE":
        # Piano: Tutto ok, assicurarsi che sia visibile sulla mappa
        plan = {
            "action": "UPDATE_DASHBOARD",
            "priority": "LOW",
            "message": f"Bici {bike_id} pronta al noleggio"
        }
